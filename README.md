...
2. 部署包内的 `config/maixcam2_puzzle_vision.json` 是横向配置，坐标左上角为 `(0, 0)`，右下角为 `(297, 210)`；左侧为源区、右侧为目标区。它初始未标定，必须在当前相机位置重新标定。
...
短按设备按键采集一帧并只求解一次，串口打印一行 JSON，屏幕保留带红色抓取点、绿色目标点和旋转箭头的结果。按键时长不小于 `800 ms` 时仅保存 `/root/app1/calibration.jpg`，不会触发求解。
