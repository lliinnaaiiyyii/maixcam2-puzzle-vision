...
- 支持题目规定尺寸内的拼图矩形：短边 `50~90 mm`、长边 `90~120 mm`，并为视觉与手工裁切误差保留 `1 mm` 容差；超出范围的候选不会输出移动命令。
...
目标矩形尺寸按题目范围验收：短边必须在 `49~91 mm`、长边必须在 `89~121 mm`（题目标称 `50~90 mm` 与 `90~120 mm`，每侧含 `1 mm` 测量容差）。例如长边 `150 mm` 的拼接方案会返回 `NO_RECTANGLE_SOLUTION`，不会保存执行方案或发送串口帧。
