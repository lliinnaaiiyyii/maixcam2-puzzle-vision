...
- 几何方案接近时，将各候选布局的源碎片重投影并比较接触带的花纹连续性；若只是同形碎片互换且最终占用区域等价，会选择一个固定方案输出，否则输出 `AMBIGUOUS` 且不输出移动记录。
...
- `AMBIGUOUS`: 多个几何或花纹方案无法可靠区分；同形碎片互换且最终矩形等价的情况会自动选定一个可执行方案，不再作为危险歧义拒绝。
